...
export function buildDynamicLeadBlock(lead = {}, extras = {}) {
  ...
  return [
    ...
    lead.aiMode === "CONTROLLED"
      ? "7. Режим CONTROLLED: по нестандартной цене, скидке или условиям ставь manager_event=decision_required."
      : "",
    "",
    "=== INTERNAL LEAD DATA ===",
...
export async function composeClientMessage({ lead, instruction }) {
  ...
  const input = [
    ...
    "Напиши одно короткое сообщение клиенту на «Вы», 2–3 предложения.",
    "Без канцелярита. Не упоминай менеджера, lead и внутренние команды.",
